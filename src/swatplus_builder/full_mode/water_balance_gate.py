...
Gate hierarchy (most fundamental → least):
  ...
  ET_DOMINATED         — ET/P > 0.70
                         Blocks: diagnostic, research_grade
                         Cause: PET_CO=1.0 (default), unrealistically high soil evap
    ...
    if et is not None and precip > 1.0:
        ...
        if ratio > _ET_P_MAX:
            conditions.append(
                ...
                f"PET_CO likely at default (1.0) — reduce to 0.3–0.6. "
                f"Blocks: diagnostic, research_grade."
            )
    return conditions
...
def _check_mass_closure(wb: dict[str, float]) -> list[str]:
    ...
    # SWAT+ stores ΔS implicitly; approximation: residual = P - (wateryld + ET + perc)
    residual = abs(precip - (wateryld + et + perc))
...
def _check_skill(nse: float, kge: Optional[float] = None) -> list[str]:
    conditions = []
...
def check_water_balance(
    ...
    *,
    ...
    if nse is not None:
        conditions += _check_skill(nse, kge)
...
def assert_tier_allowed(
    ...
    *,
    ...
    """
    result = check_water_balance(txtinout, nse=nse)
